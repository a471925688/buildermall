...
function croppers_pic(option) {

    var param = {
        area: ['900px', '600px']  //弹窗宽度
        ,mark:722/268
        ,url: $("#contextPath").val()+'/file/uploadImages'  //图片上传接口返回和（layui 的upload 模块）返回的JOSN一样
        ,done: function (url) { //上传完毕回调
            //更改图片src
            obj.parentNode.childNodes["0"].src = "temp/"+url.fileName;
        },
        croppersPath:"layui/js/"
    }
    copyObj(option,param);
//图片裁剪
    layui.config({
        base: param.croppersPath
    }).extend({
        "croppers": "croppers"
    }).use(["croppers"], function () {
        var croppers = layui.croppers;
        croppers.render(param);
    });
}
